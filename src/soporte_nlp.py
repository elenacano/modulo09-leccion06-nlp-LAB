...
class ExploracionText:
    ...
    def limpiar_textos(self, columna_limpia):
        """
        Args:
        - columna_limpia: nombre de la nueva columna donde guardaremos los datos limpios
        """
        # Aplicar limpieza
...
class RepresentacionTexto:
    ...
    def distancias_labels(self, label):
        vectorizer = CountVectorizer(max_features=self.max_features)
        # vectorizamos la columna objetivo
        X = vectorizer.fit_transform(self.df[self.text_column]).toarray()
        # calculamos sus distancias
        similarity = cosine_similarity(X)
        #matriz_similitudes = pd.DataFrame(similarity, index=self.df[label], columns=self.df[label])
